...
function serializeObject(item: any, scope: Record<string, any>): string {
    const processed = Array.isArray(item)
        ? item.map(
              (value, index) =>
                  `${serialize(index, scope)}${serialize(value, scope)}`,
          )
        : Object.keys(item).map(
              (key) => `${serialize(key, scope)}${serialize(item[key], scope)}`,
          );
    const { length } = processed.filter(
...
export default function serialize(
    ...
    if (type !== 'object') {
        throw new TypeError(
            `Unexpected type '${type}' encountered while attempting to serialize`,
        );
    }
    ...
    if (typeof item.serialize === 'function') {
        ...
        invariant(
            ...
            `${item.constructor.name}.serialize should return a string`,
        );
        return `C:${
            constructorName.length
        }:"${constructorName}":${getByteLength(
            serialized,
            options,
        )}:{${serialized}}`;
    }
    return `O:${constructorName.length}:"${constructorName}":${serializeObject(
        item,
        scope,
    )}`;
}
