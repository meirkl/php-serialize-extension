...
function getClassReference(
    ...
    invariant(
        classReference || !strict,
        `Class ${className} not found in given scope`,
    );
    if (classReference) {
        // @ts-ignore
...
function unserializeItem(
    ...
    if (type === 'string') {
        return parser.getByLength('"', '"', (length) =>
            parser.readAhead(length),
        );
    }
    if (type === 'array-object') {
        const pairs = parser.getByLength('{', '}', (length) =>
            unserializePairs(parser, length, scope, options),
        );

        const isArray = pairs.every(
            (item, idx) => isInteger(item.key) && idx === item.key,
        );
        const result: any[] | Object = isArray ? [] : {};
    ...
    if (type === 'notserializable-class') {
        const name = parser.getByLength('"', '"', (length) =>
            parser.readAhead(length),
        );
        parser.seekExpected(':');
        const pairs = parser.getByLength('{', '}', (length) =>
    ...
    if (type === 'serializable-class') {
        ...
        parser.seekExpected(':');
        const payload = parser.getByLength('{', '}', (length) =>
            parser.readAhead(length),
        );
        const result = getClassReference(name, scope, options.strict);
        if (!(result instanceof __PHP_Incomplete_Class)) {
            invariant(
                result.unserialize,
                `unserialize not found on class when processing '${name}'`,
            );
...
function unserialize(
    ...
    const parser = new Parser(
        Buffer.from(item as unknown as string),
        0,
        options,
    );
